...
const CostAwareBaseline: React.FC<Props> = ({ categories }) => {
  ...
  console.log(
    "cost-aware-baseline: certificateCategories",
    certificateCategories
  );
  console.log("cost-aware-baseline: userResponses", userResponses);
  console.log("cost-aware-baseline: certificates", certificates);
  console.log("cost-aware-baseline: company", company);
  ...
  const handleEvaluateResponses = async () => {
    try {
      const response = await apiClient.post("/assessments/", {
        certificate_id: certificates?.CAB.id,
      ...
      >
        ...
        >
          <Box display="flex" sx={{ pl: 3 }}>
            ...
            >
              Cost-Aware Baseline Certification
            </Typography>
          </Box>
          <Box
            display="flex"
        ...
        <Button
          ...
          disabled={Object.keys(userResponses).length !== 8}
...
export const getServerSideProps = async () => {
  try {
    const response = await apiClient.get(
      `categories/?type=${CERTIFICATES.COST_AWARE_BASELINE}`
    );
    ...
    console.log("Error fetching CAB categories: ", error);
    return {
      props: {
        error: "Failed fetching CAB categories",
        categories: null,
      },
    };
  }
